...
## 🔒 LANGUAGE LOCK (critical — read first)

Produce the ENTIRE mind map in the **same language and script as the input text**.
- Do NOT translate, transliterate, or switch to any other language (no Chinese, no
  Arabic, etc.) — if the input is English, every label/alias/evidence is in English.
- Keep all names and proper nouns EXACTLY as written in the source.
- The JSON keys stay in English (id, canonical, label, …); only the VALUES follow the
  input text's language.
...
## 🔒 DİL KİLİDİ (önce oku — kritik)

Tüm zihin haritasını **giriş metniyle AYNI dilde ve alfabede** üret.
- Başka bir dile ASLA çevirme, harf çevirisi yapma veya geçme (Çince, Arapça vb. yok).
  Giriş Türkçe ise her etiket/alias/kanıt Türkçe olmalı.
- Tüm özel adları metinde yazıldığı GİBİ koru.
- JSON anahtarları İngilizce kalır (id, canonical, label, …); yalnızca DEĞERLER metnin
  dilini izler.
...
⚠️ 🔒 قفل اللغة (هام — اقرأ أولًا): أنتج الخريdة كاملةً بنفس لغة وحروف النص المُدخل (العربية).
ممنوع الترجمة أو النقل الحرفي أو التحويل إلى أي لغة أخرى (إنجليزية، صينية، إلخ). كل القيم —
التسميات (label) والمرادفات (aliases) والدليل (evidence) والشروحات — يجب أن تكون بالعربية.
احتفظ بكل الأسماء كما وردت في النص تمامًا. مفاتيح JSON تبقى بالإنجليزية (id, canonical, label …)
أمّا القيم فبلغة النص.
